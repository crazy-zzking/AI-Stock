...
/// <summary>
/// 尾盘到期卖出服务：14:55 扫描未平仓持仓(tail_position)，仅处理"持有到期(hold-expired)"平仓。
/// 止损由独立的 <see cref="TailStopLossService"/> 盘中实时监控，不在此处理。
/// DryRun 下直接模拟平仓；Live 经 OrderManager 真卖。现价取当日 daily_market_snapshot 收盘/实时价。
/// </summary>
public class TailSellService
{
    ...
    /// <summary>
    /// 执行一次尾盘到期扫描：仅平仓"持有交易日数 >= HoldDays"的持仓，止损由 TailStopLossService 盘中处理。
    /// 返回平仓笔数。
    /// </summary>
    public async Task<int> RunAsync(CancellationToken ct = default)
    {
        ...
        if (opens.Count == 0)
        {
            _logger.LogInformation("尾盘到期卖出：无未平仓持仓");
            return 0;
        }
        ...
        foreach (var pos in opens)
        {
            ...
            {
                _logger.LogWarning("尾盘到期卖出跳过 {Code}：无当日现价", pos.Code);
                continue;
            }

            // 已持有交易日数 = 买入日之后出现过快照的交易日数
            ...
            // 只处理持有到期；止损由 TailStopLossService 实时监控
            if (heldDays < pos.HoldDays) continue;
            const string reason = "hold-expired";
            ...
            if (dryRun)
            {
                ...
                _logger.LogInformation(
                    "[DryRun] 尾盘到期卖出 {Code} 量={Vol}@{Price} 成本={Cost} 持有{Days}日 盈亏{Pnl}%",
                    pos.Code, pos.Volume, price, pos.BuyPrice, heldDays, pnl);
            }
            else
            {
                var req = new OrderRequest
                {
                    ...
                    StrategyName = $"{pos.Strategy}-hold-expired", SignalId = pos.SignalId,
                };
                var res = await _orderManager.PlaceOrderAsync(req);
                ok = res.Success;
                _logger.LogInformation(
                    "尾盘到期卖出 {Code} 量={Vol}@{Price} 持有{Days}日 → success={OK} msg={Msg}",
                    pos.Code, pos.Volume, price, heldDays, res.Success, res.Message);
            }
        ...
        _logger.LogInformation("尾盘到期卖出完成：平仓 {Sold}/{Total}", sold, opens.Count);
        return sold;
    }
}
